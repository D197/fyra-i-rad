...
async function bot2() {
  ...
  let all = await driver2.findElements(by.css('.player2'));
  for (let slot of all) {
    //sleep(sleepTime*1);
...
module.exports = function () {
  ...
  this.When(/^I choose to play as one human and one bot$/, async function () {
    await driver2.findElement(by.xpath('//*[@id="player_2"]')).click();
    await sleep(sleepTime * 2);
  });

  this.Then(/^the game should Start$/, async function () {
    assert.equal(await driver2.findElement(By.xpath("//*[@id='solution_header']")).getText(), "Red can win in 21 moves", "[Passed]");
  });
  ...
  this.Then(/^one should win$/, async function () {
    ...
    while (true) {
     ...
     if(result.includes('Spelare 2')){
        fs.appendFileSync("result.txt", "four-with-boten började" + " Connect four boten vann" + "\n", "UTF-8",  {'flags': 'a+'}, (err) => {
            if (err) throw err;
        })
      console.log("result saved to result.txt");
    }else if(result.includes('Spelare 1')){
        fs.appendFileSync("result.txt", "four-with-boten började" + " four-with-boten vann" + "\n", "UTF-8",  {'flags': 'a+'}, (err) => {
            if (err) throw err;
        })
      console.log("result saved to result.txt");
    }
    else{
        fs.appendFileSync("result.txt", "four-with-boten började" + " ovagjort" + "\n", "UTF-8",  {'flags': 'a+'}, (err) => {
            if (err) throw err;
        })
      console.log("result saved to result.txt");
    }
